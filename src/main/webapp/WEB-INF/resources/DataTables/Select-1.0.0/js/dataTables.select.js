...
function info ( api )
{
	...
	var add = function ( name, num ) {
		output.append( $('<span class="select-item"/>').append( api.i18n(
			'select.'+name+'s',
			{ _: '%d '+name+' selectat', 0: '', 1: '1 '+name+' selectat' },
			num
		) ) );
	};

	add( 'r&#226;nd',    api.rows( { selected: true } ).flatten().length );
	add( 'coloan&#259;', api.columns( { selected: true } ).flatten().length );
	add( 'celul&#259;',   api.cells( { selected: true } ).flatten().length );
